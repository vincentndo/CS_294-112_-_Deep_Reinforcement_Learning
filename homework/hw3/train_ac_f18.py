...
class Agent(object):
    ...
    def build_computation_graph(self):
        ...
        self.critic_update_op = tf.train.AdamOptimizer(self.learning_rate / 10).minimize(self.critic_loss)
    ...
    def estimate_advantage(self, ob_no, next_ob_no, re_n, terminal_n):
        ...
        v_n_next = self.sess.run(self.critic_prediction, feed_dict={self.sy_ob_no: next_ob_no})
        q_n = re_n + (1- terminal_n) * self.gamma * v_n_next
        adv_n = q_n - v_n
        if self.normalize_advantages:
            adv_n = skp.scale(adv_n) # YOUR_HW2 CODE_HERE
        return adv_n
    ...
    def update_critic(self, ob_no, next_ob_no, re_n, terminal_n):
        ...
        for _ in range(self.num_target_updates):
            ...
            v_n_next = self.sess.run(self.critic_prediction, feed_dict={self.sy_ob_no: next_ob_no})
            target_n = re_n + (1 - terminal_n) * self.gamma * v_n_next
