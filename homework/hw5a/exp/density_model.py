...
class RBF(Density_Model):
    ...
    def get_prob(self, states):
        ...
        if self.means is None:
            ...
            # 1. Compute deltas
            deltas = np.array( [[state - mean for mean in self.means] for state in states] )
...
class Exemplar(Density_Model):
    ...
    def make_encoder(self, state, z_size, scope, n_layers, hid_size):
        ...
        z_logstd = tf.Variable([0] * round(z_size), dtype=tf.float32)
    ...
    def get_likelihood(self, state1, state2):
        ...
        likelihood = self.sess.run( self.likelihood, feed_dict={self.state1: state1, self.state2: state2, self.discrim_target: np.ones([state1.shape[0], 1], dtype="float32")} )
        return likelihood
