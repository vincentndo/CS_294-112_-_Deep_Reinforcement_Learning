...
def build_mlp(input_placeholder, output_size, scope, n_layers, size, activation=tf.tanh, output_activation=None):
    ...
    with tf.variable_scope(scope):
        for i in range(n_layers - 1):
            output = tf.layers.dense(input_placeholder, size, activation)
            input_placeholder = output

        output_placeholder = tf.layers.dense(input_placeholder, output_size, output_activation)
...
class Agent(object):
    ...
    def build_computation_graph(self):
        ...
        if self.nn_baseline:
            ...
            baseline_loss = tf.losses.mean_squared_error(self.sy_target_n, self.baseline_prediction, scope="nn_baseline")
    ...
    def compute_advantage(self, ob_no, q_n):
        ...
        if self.nn_baseline:
            ...
            # YOUR CODE HERE
            prediction = self.sess.run(self.baseline_prediction, feed_dict={self.sy_ob_no: ob_no})
            standardized_prediction = skp.scale(prediction)
            mean, std = np.mean(q_n), np.std(q_n)
            b_n = mean + standardized_prediction * std
            adv_n = q_n - b_n
        else:
            adv_n = q_n.copy()
        return adv_n
    ...
    def estimate_return(self, ob_no, re_n):
        ...
        if self.normalize_advantages:
            ...
            adv_n = skp.scale(adv_n) # YOUR_CODE_HERE
        return q_n, adv_n
    ...
    def update_parameters(self, ob_no, ac_na, q_n, adv_n):
        ...
        if self.nn_baseline:
            ...
            # YOUR_CODE_HERE
            target_n = skp.scale(q_n)
            self.sess.run( (self.baseline_update_op, ), feed_dict={self.sy_ob_no: ob_no, self.sy_ac_na:ac_na, self.sy_adv_n: adv_n, self.sy_target_n: target_n} )
